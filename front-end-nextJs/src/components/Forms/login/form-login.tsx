...
const schema = zod.object({
  ...
  password: zod.string().min(6, { message: 'Senha deve ser informado' }),
})

type ISignInFormData = zod.infer<typeof schema>

const buildMessageError = (error: any): void => {
  ...
  ToastNotification({
    message: message || 'Ocorreu um erro interno',
    type: 'error',
  })
}
...
const FormLogin = () => {
  ...
  } = useForm<ISignInFormData>({ resolver: zodResolver(schema) })
  ...
  const onSubmit =
    async (data: ISignInFormData): Promise<void> => {
      try {
        ...
        console.error('🚀 ~ FormLogin ~ err', err)
        buildMessageError(err)
      }
    }
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="mb-5">
        <input
          ...
          value={isSubmitting ? 'Entrando...' : 'Acessar o sistema'}
